...
def train(train_dl, test_dl, epochs, out_dir, device, pretrained_model_path):
    #define model, loss and optimizer
    model = Model().to(device)
    criterion = nn.CrossEntropyLoss()

    # 加载预训练模型
    pretrained_dict = torch.load(pretrained_model_path, map_location=device)['state_dict']
...
def main():
    ...
    epochs = 5
    train(train_dl, test_dl, epochs, out_dir, device, pretrained_model_path)
